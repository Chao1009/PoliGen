...
|---|---|---|---|---|---|---|
| C-1 | *moved to Table E (§5) on 2026-09-23*: George–Knutson's η is a restricted phase-shift analysis (the paper's title), carried as a **consistency band on the quadrupole dial**, not a comparison against a measured number (`../open_items/run_2026-09-23/phase_B2_nuclear.md` §5) | — | — | — | — | — |
...
| C-3 | **μ(⁶Li) = +0.8220473 μ_N** — TUNL A = 6 | `LI6_MU_N`; `fm(0)` = (m_A/m_p)·μ | `tests/test_rc.cpp` T11 | F_m(0) = 4.90765 at 1e-4, derived | the magnetic form-factor normalisation | the q-dependence: the C0 shape and its first zero are an unfitted **model band** ([2.9, 3.3] fm⁻¹), with **no elastic ⁶Li data in this repository** to close it (the test says so in full). *2026-09-23:* the UVa FB ⁶Li row is now vendored and its C0 zero 2.694 fm⁻¹, with Li *et al.* 1971's minimum 2.828, sits below that window (`validation/benchmarks/t3_li6_charge_ff_fb.py`, recorded FAIL, nothing moved); and the magnetic slope is not free — `HoSpin1FF` fixes it with the unfitted (q_z, b) = (1.30 fm⁻¹, 1.85 fm): r_mag(⁶Li) = 2.9469 fm (measured 2026-09-23), uncompared until RFY66 / ADNDT 14 is held (`t3_li_magnetization_rfy.py`, BLOCKED) |
...
| D-3 | **POLRAD 2.0** (Akushevich *et al.*, CPC **104** (1997) 201), compiled standalone with **its own** `ffdeu` | σ^el_U at three (E, x, y) points; the sign of σ^el_T/σ^el_U; G_M(0) = 1.7139610634 (1e−7); G_Q(0) is the tree's 25.83 (1e−3), 0.04 % from POLRAD's 25.84, which is not asserted; G_C(0) = 1 is Z | `tests/test_rc.cpp` T8(c′) "POLRAD's OWN compiled deuteron numbers"; T10 "the DEUTERON fixes the (F_c, F_m, F_q) normalisation" | magnitudes gated **loosely** (0.25×–4×) because this repository uses a *different* deuteron form factor (an HO stand-in, not `ffdeu`); measured 0.883 / 0.955 / 0.390 of POLRAD's (the middle value read 0.983 until 2026-09-23, against POLRAD's stale row-2 σ_u = −1.001e−03; re-driven −1.030e−03 gives 0.9548). The two low-x points are tightened to **20 %**. Only the **sign** of σ_q/σ_u — which changes between x = 0.05 and 0.20 — is gated; its magnitude differs from POLRAD's by ×0.62 / ×0.75 / ×3.81 (different form factor) | no dropped leading minus (the sign gate). A wrong power of A (×2 at A = 2), a missing Y₊ (×2.5–2.9) or a prefactor error > 20 % is caught **only at the two x < 0.1 rows**; the 0.25×–4× x = 0.2 row passes all three; and that G_M(0) matches POLRAD's compiled value to 1e-7 | the radiative correction as physics for ⁶Li. There is **no** tensor-sector RC calculation to compare against except Gakh–Shekhovtsova (`hep-ph/0403262`), which has **zero INSPIRE citations** and supplies only the band half-width `RC_DELTA_LOW_X` (see §7). POLRAD's row 2 was re-driven 2026-09-23: σ_u = −1.0303e−03 (not −1.001e−03), σ_q/σ_u = +0.0623 (not +0.0642); the middle su/POLRAD becomes 0.955 (`../open_items/run_2026-09-23/phase_B3_chain_rc.md` §3) |
| D-4 | **eSTARlight** (Lomnitz & Klein, PRC **99** (2019) 015203) — run in-house 2026-09-02 at commit `939b11a…`, 2×10⁵ events/channel, e 10 GeV × ⁶Li 99.5 GeV/u | coherent J/ψ, φ, ρ cross sections (3 Q² windows × 2 densities), dN/d|t| slopes, and a Q²-floor scan (9 rows) | `include/lipolgen/coherent.hpp` `estarlight_li6_coherent()` / `estarlight_li6_q2_floors()`; `tests/test_coherent.cpp` "the eSTARlight 6Li baseline, and O5's reach arithmetic", "the eSTARlight 6Li Q²-floor scan"; `python/tests/test_o5_reach.py` | table values pinned at 1e-12 (they are the single code home of those numbers); the *derived* reach arithmetic at 1e-3…2e-3 | an **unpolarized rate and \|t\|-slope scale for exclusive VM production on ⁶Li** (not the coherent channel's rate: a rate scale for exclusive ρ/φ/J/ψ, not a check on the coherent channel's M_X ≥ 1.2 GeV continuum rate — 85–97 % of eSTARlight's rate (J/ψ 85.2 %, φ 95.4 %, ρ 97.2 %) is at Q² < 0.1 GeV², where `coherent.hpp` generates nothing), and the O5 feasibility arithmetic built on it (verdict: J/ψ tensor a₂ is **marginal**, 2.62 σ at 10 fb⁻¹/u, 3 σ at 13.1 fb⁻¹/u) | anything polarized. The caveats ride with every row and are stated in the header: **one spherically symmetric Gaussian density → no α+d clustering, no polarization axis, no diffractive minimum at any A; unpolarized throughout; no saturation.** A rate baseline, never an imaging one |
...
| E-18 | **the η consistency band on the quadrupole dial ↔ a₂ map (η from George–Knutson's restricted phase-shift analysis)** (composite) | the two-factor Q budget (3.317 × 2.269 = 7.524), the η ↔ dial exact linearity, the GK ±1σ mapping | `tests/test_cluster_config.cpp` T22b, T23, "the design's sec. 8 table, all three alpha-d sources" | 1e-9 … 1e-13 | that the ⁶Li quadrupole is an **explicitly dialled, banded** input and that every downstream number (a₂, ε_b0, P_D^{αd}) moves with it coherently | it is a *bookkeeping* validation of a model band, not a physics check |
| C-1 | **η(⁶Li → α+d) = −0.025 ± 0.006(stat) ± 0.010(syst)** — E. A. George, L. D. Knutson, PRC **59** (1999) 598, by a restricted phase-shift analysis (the paper's title; not a direct d+α tensor-analysing-power measurement) — *in Table C until 2026-09-23* | `ClusterConfigSampler::asymptotic_ds_ratio()`, `LI6_ETA_DS_GK{,_STAT,_SYST}` | `tests/test_cluster_config.cpp` T22 "the asymptotic D/S ratio, Whittaker-divided"; T22b "eta rides the quadrupole dial" | a **consistency band on the quadrupole dial**, not a D/S benchmark: η is exactly linear in the dial (T22b), and T22 asserts `1.2 < |η/η_GK| < 3.0`, pinning a documented 1.93× distance of the UNDIALLED tables (model **−0.0482**) | that the α–d D/S asymptotic ratio is computed correctly (Whittaker-divided, not the naive R₂/R₀ = −0.149) and that the undialled tables sit ≈ 2× — not 5–15× — from the phase-shift-analysis η | anything about the ⁶Li tensor observables themselves, or the α–d D wave as physics. T22b shows η is **exactly linear** in the quadrupole dial, so it is a relabelling of that dial, and GK's ±1σ band maps onto a Q(⁶Li) interval that **crosses zero** |
...
C-6 (Machleidt Table XX), C-1 (T22 η, a consistency band on the quadrupole dial), D-4 (eSTARlight exclusive-VM rate scale — 85–97 % of its rate at Q² < 0.1 GeV², below `coherent.hpp`'s q2_min = 0.7), and T1r.
...
about saying so — several of its "gates" (C-1 — a consistency band on a dial, not a gate on data —, C-2, E-13/T6, E-16) exist to pin a
